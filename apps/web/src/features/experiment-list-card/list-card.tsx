import type { ExperimentDto, ExperimentStatus } from '@labcollab/shared';
...
const TEXT_FIELDS: { key: keyof ExperimentDto; label: string }[] = [
...
export function ExperimentListCard({ experiment }: ExperimentListCardProps) {
  ...
  const fields = allFields.slice(0, MAX_FIELD_LINES);
    ...
    >
      <Group align="flex-start" wrap="wrap" gap="lg">
          ...
          <Stack gap="xs">
            ...
            {hiddenFieldsCount > 0 && (
              <Text size="xs" c="dimmed">
                Ещё {hiddenFieldsCount} заполненных полей
              </Text>
            )}
          </Stack>
        )}
        ...
        <Stack gap={2} align="flex-end" mt="auto">
          ...
          <Text size="xs" c="dimmed">
            Изменён {formatDateTime(experiment.updatedAt)}
          </Text>
          {experiment.conductedAt && (
            <Text size="xs" c="dimmed">
              Проведён {formatDateTime(experiment.conductedAt)}
            </Text>
          )}
        </Stack>
      </Stack>
