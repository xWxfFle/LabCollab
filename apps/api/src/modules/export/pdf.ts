...
export async function buildExperimentPdf(
  ...
  if (dto.hypothesis) content.push({ text: `Гипотеза: ${dto.hypothesis}`, margin: [0, 4, 0, 0] });
  if (dto.materials) content.push({ text: `Материалы: ${dto.materials}`, margin: [0, 4, 0, 0] });
  if (dto.protocolSteps)
    content.push({ text: `Протокол: ${dto.protocolSteps}`, margin: [0, 4, 0, 0] });
  if (dto.conditions) content.push({ text: `Условия: ${dto.conditions}`, margin: [0, 4, 0, 0] });
  if (dto.results) content.push({ text: `Результаты: ${dto.results}`, margin: [0, 4, 0, 0] });
  if (observationsText)
    content.push({
